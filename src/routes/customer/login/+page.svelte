<script lang="ts">
	...
	import { StatusBar, AppBar, Btn, Field, Banner, Brand, Footer, HomeIndicator, OtpInput, Icon } from '$lib/components/index.js';
	import { requestOtp, setPin } from '$lib/api/auth';
	import { auth, login, setHasPin } from '$lib/stores';

	let step = $state<'phone' | 'pin' | 'otp' | 'setpin'>('phone');
	...
	async function handleRequestOtp() {
		...
		loading = false;
		if (!result.ok) { error = 'Não foi possível enviar o código. Tente de novo.'; return; }
		step = 'otp';
	}
	...
	async function handleResend() {
		...
		const result = await requestOtp(phoneNumber);
		if (!result.ok) { error = 'Não foi possível reenviar o código.'; return; }
		resendTimer = 30;
	}
