...
<div class="mz-banner" style:background={bg}>
	{#if icon}<Icon name={icon} size={17} stroke={2} style="color:{fg};flex:0 0 auto;margin-top:1px" />{/if}
	<div>
		{#if title}<div style="font-weight:700;font-size:13px;color:{fg};margin-bottom:2px">{title}</div>{/if}
		<div style="font-size:12px;color:var(--ink-2);line-height:1.45">{@render children?.()}</div>
	</div>
</div>
